...
def get_action_index(readout_t, epsilon, t):
    ...
    action_index = 0
    if(t < OBSERVE):
        action_index = np.random.randint(0, 3, 1, dtype=int)
    else:
...
def compute_target_q(r_batch, readout_j1_batch, terminal_batch):
    ...
    for i in range(0, len(terminal_batch)):
        ...
        if terminal_batch[i]:
            ...
            q_val = r_batch[i] + GAMMA*np.max(readout_j1_batch[i])
            target_q_batch.append(q_val)

    target_q_batch = np.array(target_q_batch).reshape((len(terminal_batch), 1))
